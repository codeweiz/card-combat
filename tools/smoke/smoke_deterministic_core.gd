...
const BoardState = preload("res://src/core/simulation/board_state.gd")
...
const CardEffectResolver = preload("res://src/core/card/card_effect_resolver.gd")
const CancelOriginalEffectParams = preload("res://src/core/card/cancel_original_effect_params.gd")
const EffectRef = preload("res://src/core/card/effect_ref.gd")
...
const ResponseWindowState = preload("res://src/core/simulation/response_window_state.gd")
...
func _run() -> int:
	...
	var player_ids: Array[StringName] = setup.player_ids
	var card_data_hash: String = setup.card_data_hash
	var player_a_loadout: PlayerDeckLoadout = _build_loadout(player_ids[0], &"sample_a", card_data_hash, &"vanguard", 20)
	var player_b_loadout: PlayerDeckLoadout = _build_loadout(player_ids[1], &"sample_b", card_data_hash, &"vanguard", 20)
	setup.player_deck_loadouts = [player_a_loadout, player_b_loadout]
...
func _build_loadout(player_id: StringName, deck_id: StringName, card_data_hash: String, card_id: StringName, deck_size: int) -> PlayerDeckLoadout:
	var loadout := PlayerDeckLoadout.new()
	loadout.player_id = player_id
	loadout.deck_id = deck_id
	loadout.deck_fingerprint = &""
	loadout.ordered_card_ids = []
	loadout.declared_archetype_tags = []
	for i in deck_size:
...
func _run_one_sequence(setup: MatchSetup, card_database: CardDatabase) -> String:
	var core: DeterministicSimulationCore = DeterministicSimulationCore.new()
	var init_result: SimulationResult = core.initialize_match(setup, card_database)
	...
	var play_unit: ActionCommand = _build_play_unit_command(
		&"player_a",
		player_a_seq,
		&"vanguard",
	...
	var play_result: SimulationResult = core.submit_command(play_unit)
	...
	var noop_blocked: ActionCommand = ActionCommand.new()
	...
	var noop_blocked_result: SimulationResult = core.submit_command(noop_blocked)
	if noop_blocked_result.accepted:
		push_error("Active player should not act while defender has response priority")
		return ""

	var pass_response: ActionCommand = _build_pass_response_command(
		&"player_b",
		player_b_seq,
		core.get_state_hash()
	)
	var pass_result: SimulationResult = core.submit_command(pass_response)
	...
	var enter_attack: ActionCommand = _build_end_turn_command(
		&"player_a",
		player_a_seq,
		core.get_state_hash(),
		&"p1_end_to_attack"
	)
	var enter_attack_result: SimulationResult = core.submit_command(enter_attack)
	...
	var illegal_play: ActionCommand = _build_play_unit_command(
		&"player_a",
		player_a_seq,
		&"vanguard",
	...
	var illegal_play_result: SimulationResult = core.submit_command(illegal_play)
	if illegal_play_result.accepted:
		push_error("play_unit should be invalid in attack phase")
		return ""
	var illegal_end_b: ActionCommand = _build_end_turn_command(
		&"player_b",
		player_b_seq,
		core.get_state_hash(),
		&"p1_illegal_end_b"
	)
	var illegal_end_b_result: SimulationResult = core.submit_command(illegal_end_b)
	...
	var finish_attack: ActionCommand = _build_end_turn_command(
		&"player_a",
		player_a_seq,
		core.get_state_hash(),
		&"p1_end_attack"
	)
	var finish_attack_result: SimulationResult = core.submit_command(finish_attack)
	...
	var b_to_attack: ActionCommand = _build_end_turn_command(
		&"player_b",
		player_b_seq,
		core.get_state_hash(),
		&"p1_b_end_to_attack"
	...
	player_b_seq += 1
	var b_to_attack_result: SimulationResult = core.submit_command(b_to_attack)
	if not b_to_attack_result.accepted:
		push_error("B failed to end main phase: %s" % b_to_attack_result.message)
		return ""
	var b_to_next: ActionCommand = _build_end_turn_command(
		&"player_b",
		player_b_seq,
		core.get_state_hash(),
		&"p1_b_end_attack"
	...
	player_b_seq += 1
	var b_to_next_result: SimulationResult = core.submit_command(b_to_next)
	...
	var a_end_to_attack_again: ActionCommand = _build_end_turn_command(
		&"player_a",
		player_a_seq,
		core.get_state_hash(),
		&"p1_a_end_to_attack_again"
	)
	var a_end_to_attack_again_result: SimulationResult = core.submit_command(a_end_to_attack_again)
	...
	if ready_attacks.is_empty():
		...
		return ""
	var attack: ActionCommand = ready_attacks[0]
	...
	player_a_seq += 1
	var attack_result: SimulationResult = core.submit_command(attack)
	...
	if not _response_window_open(core):
		...
		return ""
	var attack_pass: ActionCommand = _build_pass_response_command(
		&"player_b",
		player_b_seq,
		core.get_state_hash()
	)
	player_b_seq += 1
	var attack_pass_result: SimulationResult = core.submit_command(attack_pass)
	...
	var duplicate: ActionCommand = _build_play_unit_command(
		&"player_a",
		player_a_seq,
		&"vanguard",
		BoardState.LANE_CENTER,
	...
	var duplicate_result: SimulationResult = core.submit_command(duplicate)
...
func _response_window_open(core: DeterministicSimulationCore) -> bool:
	...
	var response_window: Dictionary = _dict_get(snapshot, "response_window", {})
	return _dict_get(response_window, "window_state", &"") == ResponseWindowState.STATE_OPEN
...
func _get_turn_phase(core: DeterministicSimulationCore) -> StringName:
	var snapshot: Dictionary = core.get_state_snapshot()
	return _dict_get(snapshot, "turn_phase", &"")
...
func _get_player_life(core: DeterministicSimulationCore, player_id: StringName) -> int:
	...
	var player_life_by_id: Dictionary = _dict_get(board, "player_life_by_id", {})
	return int(_dict_get(player_life_by_id, str(player_id), 0))
...
func _player_has_unit_in_lane(core: DeterministicSimulationCore, lane_id: StringName, player_id: StringName) -> bool:
	var snapshot: Dictionary = core.get_state_snapshot()
	var board: Dictionary = _dict_get(snapshot, "board", {})
	var lanes: Array = _dict_get(board, "lanes", [])
	for lane_variant: Variant in lanes:
		var lane: Dictionary = lane_variant
		if _dict_get(lane, "lane_id", &"") == lane_id:
			var units_by_player: Dictionary = _dict_get(lane, "units_by_player", {})
			return _dict_get(units_by_player, str(player_id), null) != null
	return false
